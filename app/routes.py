...
def api_enroll_save():
    if not state.temp_face_enc: return jsonify({'ok': False})
    data = request.json
    try:
        stu = supabase.table('students').insert({'name': data['name'], 'roll_no': data['roll_no'], 'class_name': data['class']}).execute().data[0]
